...
func (t *EnterpriseUsersTool) GetToolDefinition() mcp.Tool {
	return mcp.NewTool(t.name,
		...
		mcp.WithString("status",
			mcp.Description("用户状态过滤 (active, inactive, pending)"),
		),
		mcp.WithString("department",
			mcp.Description("部门过滤"),
		),
		mcp.WithString("role",
			mcp.Description("角色过滤"),
		),
	)
}
...
func (t *CreateEnterpriseUserTool) GetToolDefinition() mcp.Tool {
	return mcp.NewTool(t.name,
		mcp.WithDescription(t.description),
		mcp.WithString("name",
			mcp.Description("用户登录名"),
			mcp.Required(),
		),
		mcp.WithString("display_name",
			mcp.Description("用户显示名称"),
			mcp.Required(),
		),
		mcp.WithString("email",
			mcp.Description("用户邮箱地址"),
			mcp.Required(),
		),
		mcp.WithString("mobile",
			mcp.Description("用户手机号码"),
		),
		mcp.WithString("department",
			mcp.Description("用户所属部门"),
		),
		mcp.WithString("job",
			mcp.Description("用户职位"),
		),
		mcp.WithString("employee_number",
			mcp.Description("员工编号"),
		),
		mcp.WithString("password",
			mcp.Description("用户密码"),
		),
	)
}
